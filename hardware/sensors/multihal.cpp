...
#define RETRY_SLEEP_TIME_SECONDS 1
...
static int open_sensors(const struct hw_module_t* hw_module, const char* name,
    ...
    int i;
    int retryCount = 20;
    ALOGE("open_sensors begin...");

    for (i=0; i < RETRY_COUNT; ++i){
         ALOGE("Check for file round: %d...", i);
         if( access( SOCKET_PATH, F_OK ) != -1 ) {
              ALOGE("sensors.qcom socket <%s> exists. Continue with sensor loading....", SOCKET_PATH );
              break;
         } else {
              ALOGE("File <%s> doesn't exists. Waiting....", SOCKET_PATH );
              sleep( RETRY_SLEEP_TIME_SECONDS );
         }
    }
